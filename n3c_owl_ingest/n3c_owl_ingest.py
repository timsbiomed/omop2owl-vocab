...
def via_yarrrml(retain_intermediates=True):
    ...
    print('Conversion to RML mapping template complete. Output from YARRRML -> RML command: ')
    print(results1.stdout.decode())

    # Use the RML mapping template to process the csv's and output the triples as a ttl file.
    # TODO: any way to get this as OWL? If not, can semsql use TTL? if not, can convert using robot, but will probably
    #  run out of memory
    ...
    print('Conversion from RML mapping template to nquads complete. Output from RML Mapper command: ')
    print(results2.stdout.decode())
